...
@property (weak, nonatomic) IBOutlet UILabel *guessWordLbl;
...
-(void) createHiddenWord {
    ...
    _curGuessCount = 0;
    _hangmanImageView.image = [UIImage imageNamed:@"0.png"];
...
-(void)UpdateWordWithLetter:(unichar)letter {
    ...
    if(found == NO) {
        ...
        if(_curGuessCount <= 10) {
            _hangmanImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"%d",_curGuessCount]];
        }
        else {
